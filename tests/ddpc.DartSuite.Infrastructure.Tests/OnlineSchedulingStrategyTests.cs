...
public sealed class OnlineSchedulingStrategyTests
{
    ...
    public async Task LockedBoardMatches_DoNotOverlap()
    {
        ...
        var end1 = r1.PlannedStartUtc!.Value.AddMinutes(15);
        // r2 must start at or after r1 ends (no overlap on same board)
        r2.PlannedStartUtc!.Value.Should().BeOnOrAfter(end1);
    }
